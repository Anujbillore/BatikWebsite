...
export default function WhatsAppButton() {
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      aria-label="Chat with Megha on WhatsApp"
      className="fixed z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_0_28px_rgba(37,211,102,0.45)] transition hover:scale-105 md:h-auto md:w-auto md:gap-3 md:px-4 md:py-3 md:text-sm md:font-semibold"
      style={{
        bottom: "max(1rem, env(safe-area-inset-bottom))",
        right: "max(1rem, env(safe-area-inset-right))",
      }}
    >
      <svg viewBox="0 0 24 24" className="h-6 w-6 fill-current md:h-5 md:w-5" aria-hidden>
      ...
      <span className="hidden md:inline">Chat Megha</span>
    </a>
  );
}
